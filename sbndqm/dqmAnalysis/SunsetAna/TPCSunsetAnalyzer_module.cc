...
void sunsetAna::TPCSunsetAnalyzer::analyze(art::Event const& e)
{
  ...
  auto level = 3;
  std::string groupName = "Sunset";
  std::string chID = "0"; // only 1 channel
  ...
  if (digit_handle.isValid() && !digit_handle->empty()) {
    ...
    sbndaq::sendMetric(groupName, chID, "ndigi", ndigi_noise, level, artdaq::MetricMode::LastPoint);

    bool isSunset = isSunsetEvent(*digit_handle);
    if (isSunset==false){
      getBaselines(*digit_handle, fBaselines);
      sbndaq::sendMetric(groupName, chID, "nspikes", 0, level, artdaq::MetricMode::LastPoint);
    }
    else{
      ...
      sbndaq::sendMetric(groupName, chID, "nspikes", nspikes, level, artdaq::MetricMode::LastPoint);
