...
class tpcAnalysis::OfflineEvd : public art::EDAnalyzer {
public:
  explicit OfflineEvd(fhicl::ParameterSet const& p);
  // The compiler-generated destructor is fine for non-base
  // classes without bare pointers or other resource use.

  // Plugins should not be copied or assigned.
  OfflineEvd(OfflineEvd const&) = delete;
  OfflineEvd(OfflineEvd&&) = delete;
  OfflineEvd& operator=(OfflineEvd const&) = delete;
  OfflineEvd& operator=(OfflineEvd&&) = delete;
...
tpcAnalysis::OfflineEvd::OfflineEvd(fhicl::ParameterSet const& p)
  : EDAnalyzer{p}
...
void tpcAnalysis::OfflineEvd::SendImages(art::Event const& e)
{
...
void tpcAnalysis::OfflineEvd::analyze(art::Event const& e)
{
  ...
  for (const auto & rd : _raw_digits_handle){
    ...
    for (unsigned short i = 0; i<adc_vec.size(); ++i){
      //std::cout << "[tpcAnalysis::OfflineEvd::analyze] adc_vec[" << i << "] : " << adc_vec[i] << ", rd->GetPedestal() : " << rd->GetPedestal() << std::endl;
...
void tpcAnalysis::OfflineEvd::beginJob()
{
...
DEFINE_ART_MODULE(tpcAnalysis::OfflineEvd)
