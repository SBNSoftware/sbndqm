...
float tpcAnalysis::ChannelData::meanPeakHeight() {
  if (peaks.size() == 0) {
    return 0;
  } 
...
float tpcAnalysis::ChannelData::meanPeakHeight(const std::vector<art::Ptr<recob::Hit> > &hits) {
  if(hits.size() == 0) {
    return 0;
  }
  int total = 0;
...
// only count up peaks
float tpcAnalysis::ChannelData::Occupancy() {
  float n_peaks = 0;
  for (unsigned i = 0; i < peaks.size(); i++) {
    if (peaks[i].is_up) {
      n_peaks += 1;
