...
void tpcAnalysis::OnlineEvd::SendImages(art::Event const& e)
{
  ...
  for (int i = 0; i<6; ++i){
    ...
    //std::cout << "[tpcAnalysis::OnlineEvd::SendImages] " << image_path << " : " << image_data.data() << std::endl;
    //std::cout << "[tpcAnalysis::OnlineEvd::SendImages] " << image_path << " : " << image_data << std::endl;
...
void tpcAnalysis::OnlineEvd::analyze(art::Event const& e)
{
  ...
  for (int i = 0; i<6; ++i){
    int tpc = i/3;
    int plane = i%3;
    hrawadc[i] = new TH2D(Form("hrawadc%d",i),Form("Run %d Event %d TPC %d Plane %d;Wire;Tick;ADC", run, event, tpc, plane), nbins[plane], 0, nbins[plane]-1, 6000, 0, 6000);
    if (plane == 2){
         qmax = 150;
         qmin = -50;
    }
