...
namespace tpcAnalysis {
  class OnlineAnalysisSBND;
}


class tpcAnalysis::OnlineAnalysisSBND : public art::EDAnalyzer {
public:
  explicit OnlineAnalysisSBND(fhicl::ParameterSet const & p);
  // The compiler-generated destructor is fine for non-base
  // classes without bare pointers or other resource use.

  // Plugins should not be copied or assigned.
  OnlineAnalysisSBND(OnlineAnalysisSBND const &) = delete;
  OnlineAnalysisSBND(OnlineAnalysisSBND &&) = delete;
  OnlineAnalysisSBND & operator = (OnlineAnalysisSBND const &) = delete;
  OnlineAnalysisSBND & operator = (OnlineAnalysisSBND &&) = delete;
...
tpcAnalysis::OnlineAnalysisSBND::OnlineAnalysisSBND(fhicl::ParameterSet const & p):
...
void tpcAnalysis::OnlineAnalysisSBND::analyze(art::Event const & e) {
  // UNIX time in ms
...
void tpcAnalysis::OnlineAnalysisSBND::SendCorrelationMatrix(const art::Event &e) {
...
void tpcAnalysis::OnlineAnalysisSBND::SendWaveforms(const art::Event &e) {
...
void tpcAnalysis::OnlineAnalysisSBND::SendTimeAvgFFTs(const art::Event &e) {
...
void tpcAnalysis::OnlineAnalysisSBND::SendFFTs(const art::Event &e) {
  for (const ChannelDataSBND &chan: _analysis._per_channel_data) {
...
void tpcAnalysis::OnlineAnalysisSBND::SendSparseWaveforms(const art::Event &e) {
  ...
  for (unsigned channel = 0; channel < _analysis._per_channel_data.size(); channel++) {
    const ChannelDataSBND &data = _analysis._per_channel_data[channel];
...
DEFINE_ART_MODULE(tpcAnalysis::OnlineAnalysisSBND)
